...
void aes_ecb_test()
{
    uint8_t key_vector[32] = "\x2b\x7e\x15\x16\x28\xae\xd2\xa6\xab\xf7\x15\x88\x09\xcf\x4f\x3c\x2b\x7e\x15\x16\x28\xae\xd2\xa6\xab\xf7\x15\x88\x09\xcf\x4f\x3c";
    uint8_t pt_vector[32] =  "\x6b\xc1\xbe\xe2\x2e\x40\x9f\x96\xe9\x3d\x7e\x11\x73\x93\x17\x2a\x6b\xc1\xbe\xe2\x2e\x40\x9f\x96\xe9\x3d\x7e\x11\x73\x93\x17\x2a";

    uint8_t ct_vector[32] =  "\x3a\xd7\x7b\xb4\x0d\x7a\x36\x60\xa8\x9e\xca\xf3\x24\x66\xef\x97\x3a\xd7\x7b\xb4\x0d\x7a\x36\x60\xa8\x9e\xca\xf3\x24\x66\xef\x97";
    uint8_t output[32];
    uint8_t input[32];
    
    printf("AES ECB\n");

    aesm_ecb_encrypt(output, pt_vector, m_vector, 32,key_vector);


    printf("cipher text: \n");
    dump_hex(output, 32);

    aes_ecb_decrypt(input, output, 32, key_vector);

    printf("plain text: \n");
    dump_hex((uint8_t * )input,32);

    if (memcmp(pt_vector, input, 32) != 0)
    {
        fprintf(stderr,"error: decrypted ciphertext is not the same as the input plaintext\n");
        exit(1);
    }
    else 
        if (memcmp(ct_vector, output, 32) != 0)
    {
